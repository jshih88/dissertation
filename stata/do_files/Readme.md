...
3. [js_all_data_regress_iq8_exposures_age_sex_ses.do](#js_all_data_regress_iq8_exposures_age_sex_ses.do)
4. [js_all_data_regress_iq8_exposures_plot_by_age.do](#js_all_data_regress_iq8_exposures_plot_by_age.do)
5. [js_all_data_regress_iq8_exposures_plot_by_depvars.do](#js_all_data_regress_iq8_exposures_plot_by_depvars.do)
...
### Relationship to Other Files
This file is included by `js_all_data_regress_iq8_exposures_age_sex_ses.do` to ensure the SES variable is available as a covariate in the regression analyses. The SES variable is critical for the main analysis as it allows the researchers to control for socioeconomic factors when examining the relationship between childhood IQ and cardiovascular risk factors.

## js_all_data_regress_iq8_exposures_age_sex_ses.do

### Overview
`js_all_data_regress_iq8_exposures_age_sex_ses.do` is the core analytical script that performs multiple linear regression analyses to examine the relationship between childhood IQ at age 8 and various cardiovascular risk factors across different ages, while controlling for age, sex, and socioeconomic status (SES).
...
### Inputs
- CSV file: "output\js_all_data_regress_iq8_results.csv" containing regression results from the main analysis
...
### Relationship to Other Files
- Uses the results generated by `js_all_data_regress_iq8_exposures_age_sex_ses.do`
- Complements `js_all_data_regress_iq8_exposures_plot_by_depvars.do` by providing an alternative visualisation approach
- While `js_all_data_regress_iq8_exposures_plot_by_depvars.do` organises plots by dependent variable (showing age trends), this script organises plots by age (showing all dependent variables at each age)

## js_all_data_regress_iq8_exposures_plot_by_depvars.do

### Overview
`js_all_data_regress_iq8_exposures_plot_by_depvars.do` is a visualisation script that creates plots showing the association between IQ at age 8 and various cardiovascular risk factors across different ages. Unlike the plot_by_age script, this file organises the visualisation by dependent variable, allowing for the examination of age trends for each specific cardiovascular risk factor.
...
### Outputs
- PNG file: "output\js_all_data_regress_iq8_exposures_plot_by_depvars.png" showing the association between IQ at age 8 and each cardiovascular risk factor across all available ages

### Relationship to Other Files
- Uses the results generated by `js_all_data_regress_iq8_exposures_age_sex_ses.do`
- Complements `js_all_data_regress_iq8_exposures_plot_by_age.do` by providing an alternative visualisation approach
- While `js_all_data_regress_iq8_exposures_plot_by_age.do` organises plots by age (showing all dependent variables at each age), this script organises plots by dependent variable (showing age trends for each cardiovascular risk factor)
...
3. **Core Statistical Analysis** (`js_all_data_regress_iq8_exposures_age_sex_ses.do`):
...
4. **Age-Specific Visualisation** (`js_all_data_regress_iq8_exposures_plot_by_age.do`):
...
5. **Variable-Specific Visualisation** (`js_all_data_regress_iq8_exposures_plot_by_depvars.do`):
   - Imports the regression results
