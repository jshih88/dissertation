...
foreach vexpo of local exposure_vars {
	...
	foreach viq of local iq_vars {
		...
		capture mkdir "output"
		capture log close
		log using "output/js_all_data_regress_iq8_`vexpo'_age_sex_ses.log", replace
		...
		foreach vage of local `vexpo'_age_vars {
			...
			di "==============================================================================="
			di "Regression on: [regress z_`vexpo'_`vage' total_iq_`viq' age_`vage' i.sex i.ses]"
			di "==============================================================================="
			regress z_`vexpo'_`vage' total_iq_`viq' age_`vage' i.sex i.ses
			...
			di "Age: " `vage'
			di "IQ Coefficient: " results[1,2]
			di "IQ 95% CI Lower: " results[5,2]
			di "IQ 95% CI Upper: " results[6,2]
			di "Regression results summary:", "IQ_at: " `viq', ", Exposure: " "`label_text'_(`vexpo')", ", Age: " `vage', ", IQ_Coefficient: " results[1,2], ", IQ_95%_CI_Lower: " results[5,2], ", IQ_95%_CI_Upper: " results[6,2]
			di "==========================="
		...
		twoway ///
			...
			title("Combined Association: [Childhood IQ at `viq'] and [`label_text'] Across Ages with ALL data", size(medium) span) ///
		...
		graph export "output/js_all_data_regress_iq8_`vexpo'_age_sex_ses.png", width(2000) height(1200) replace
